...
const tempAssetId = mongoose.Types.ObjectId();
...
const sampleAssetData = {
  ...
  modelNumber: "CHR-001",
  desc: "Gaming Chair",
  assetType: mongoose.Types.ObjectId(),
...
const sampleEditAssetData = {
  assetId: tempAssetId,
  name: "Chair - edit",
  brandName: "INFOR - edit",
  modelNumber: "CHR-001 - edit",
  desc: "Gaming Chair - edit",
  assetType: mongoose.Types.ObjectId(),
  createdBy: mongoose.Types.ObjectId(),
  createdAt: Date.now(),
  updatedBy: mongoose.Types.ObjectId(),
...
describe("TESTING /api/asset", () => {
  ...
  describe("[GET]", () => {
    ...
    it("GET ASSET BY ASSET ID", (done) => {
      request(server)
        .get("/api/asset/" + tempAssetId)
        .set({ Authorization: `Bearer ${userAdmin.accessToken}` })
        .end((err, response) => {
          expect(response.status).to.equal(200);
          expect(response).to.contain.property("text");
  ...
  describe("[PUT]", () => {
    it("UPDATE AN ASSET", (done) => {
      request(server)
        .put("/api/asset")
        .set({ Authorization: `Bearer ${userAdmin.accessToken}` })
        .send({ params: sampleEditAssetData })
        .end((err, response) => {
          expect(response.status).to.equal(200);
          expect(response).to.contain.property("text");

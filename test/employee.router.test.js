...
const tempEmployeeId = mongoose.Types.ObjectId();
...
describe("TESTING /api/employee", () => {
  ...
  describe("[POST]", () => {
    it("CREATE AN EMPLOYEE", (done) => {
      request(server)
        ...
        .send({ params: sampleEmployeeData })
        .end((err, response) => {
  ...
  describe("[GET]", () => {
    ...
    it("GET EMPLOYEE BY EMPLOYEE ID", (done) => {
      request(server)
        .get("/api/employee/" + tempEmployeeId)
        .set({ Authorization: `Bearer ${userAdmin.accessToken}` })
        .end((err, response) => {
          expect(response.status).to.equal(200);
          expect(response).to.contain.property("text");
  ...
  describe("[PUT]", () => {
    it("UPDATE AN EMPLOYEE", (done) => {
      request(server)
        .put("/api/employee")
        .set({ Authorization: `Bearer ${userAdmin.accessToken}` })
        .send({ params: sampleEditEmployeeData })
        .end((err, response) => {
          expect(response.status).to.equal(200);
          expect(response).to.contain.property("text");
  ...
  describe("[DELETE]", () => {
    it("DELETE AN EMPLOYEE", (done) => {
      request(server)
        .delete("/api/employee/" + tempEmployeeId)
        .set({ Authorization: `Bearer ${userAdmin.accessToken}` })
        .end((err, response) => {
          expect(response.status).to.equal(200);
          expect(response).to.contain.property("text");

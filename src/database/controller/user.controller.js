...
exports.getUser = async function (userId) {
  if (!userId) throw global.ErrorBadRequest("User id not found");
  ...
  const user = await userModel
    ...
    .populate("role")
    .exec();
  if (!user) throw global.ErrorDataNotFound("User not found");
...
exports.registerUser = async function (userData) {
  if (!userData.username) throw global.ErrorBadRequest("Username not found");
  if (!userData.password) throw global.ErrorBadRequest("Password not found");
  if (!userData.confirmPassword)
    throw global.ErrorBadRequest("Confirmation password not found");
  if (userData.password !== userData.confirmPassword)
    throw global.ErrorBadRequest("Confirmation password is incorrect");
  if (!userData.email) throw global.ErrorBadRequest("Email not found");
  if (!validateEmail(userData.email))
    throw global.ErrorBadRequest("Email not valid");
  if (!userData.fullname) throw global.ErrorBadRequest("Fullname not found");
...
exports.createUser = async function (userData) {
  ...
  if (!userData.role) throw global.ErrorBadRequest("Role id not found");
  const role = await roleController.getRole(userData.role);
  if (!role) throw global.ErrorBadRequest("Role id is invalid");
...
exports.updateUser = async function (userData) {
  if (!userData._id) throw global.ErrorBadRequest("User id not found");
...
exports.deleteUser = async function (userId) {
  ...
  if (res.n === 0) throw global.ErrorDataNotFound("User not found");
...
exports.changeUserUsername = async function (userData) {
  ...
  if (user.username === userData.username) return userData;
  const isUsernameAvailable = await exports.isUsernameAvailable(
    userData.username,
    user._id
  );
  if (!isUsernameAvailable) throw global.ErrorBadRequest("Username is taken");
